...
namespace tuw
{
  ...
  {
  public:
    WorldFile() : resolution_x(.0), rotation_y(.0), rotation_x(.0), coordinate_x(.0), coordinate_y(.0) {}
  ...
  public:
    ...
    {
      GeographicLib::UTMUPS::Forward(lla[0], lla[1], this->zone_, this->northp_, this->utm_[0], this->utm_[1]);
      this->utm_[2] = lla[3];
     ...
     **/
    void init(cv::Size canvas_size, double resolution, Origin origin, cv::Vec3d utm, int zone, bool northp)
    {
      this->utm_ = utm;
      this->zone_ = zone;
      this->northp_ = northp;
    ...
    cv::Vec3d &lla2utm(const cv::Vec3d &src, cv::Vec3d &des) const
    {
      ...
      double gamma, k;
      bool northp;
      GeographicLib::UTMUPS::Forward(src[0], src[1], zone, northp, des[0], des[1], gamma, k, this->zone_);
      des[2] = src[2];
      return des;
    }
    /**
