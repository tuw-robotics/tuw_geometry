...
double GeoMapMetaData::dx() const {return size.width * resolution;}
double GeoMapMetaData::dy() const {return size.height * resolution;}
std::string GeoMapMetaData::info_map() const
{
  char txt[0x1FF];
  sprintf(
    txt,
    "MapInfo: [%4dpix, %4dpix] * %6.5f m/pix = [%6.2fm, %6.2fm]; origin: [%6.2fm, %6.2fm, "
    "%4.3frad]",
    size.width, size.height, resolution, dx(), dy(), origin.x(), origin.y(), origin.theta());
  return txt;
}
...
std::string GeoMapMetaData::info_geo() const
{
  ...
  sprintf(
    ...
    origin_world_lla[0], origin_world_lla[1], origin_world_lla[2], utm_offset[0], utm_offset[1],
    utm_offset[2], utm_zone, (utm_northp ? "north" : "south"));
  return txt;
}
